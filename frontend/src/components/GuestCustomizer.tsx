...
export default function GuestCustomizer({ onBack }: GuestCustomizerProps) {
  ...
  const handleGuestLogin = async () => {
    if (!name.trim() || isLoadingGuest) return;
    ...
    <div
      ...
      <div
        ...
        <BackButton
          ...
          size={24}
        />
        <h2 className={styles.title}>{t("Home.customizeProfileTitle")}</h2>
          ...
          <div
            ...
            {avatarOptions.map((src, index) => (
              <Image
                key={src}
                src={src}
                alt={t("Home.avatarOption", { index: index + 1 })}
                width={120}
                height={120}
          ...
          <div
            ...
            {skinOptions.map((s) => (
              <Image
                ...
                data-testid={`skin-option-${s.value.replaceAll("/", "-")}`}
              />
            ))}
          </div>
        </div>
        ...
        <button
          ...
          disabled={!name.trim() || isLoadingGuest}
          aria-busy={isLoadingGuest}

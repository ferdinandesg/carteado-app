...
const TrucoButton = withSound(Button, {
...
export default function TrucoActions() {
  ...
  const isTrucoPending = Boolean(
    game?.trucoAskerId && game?.trucoState === "PENDING"
  );
  const isTrucoAskedByMyTeam = myTeam?.userIds.includes(
    game?.trucoAskerId || ""
  );
